...
const firstName = computed(
  () => order.value?.user?.firstName + " " + order.value?.user?.lastName || ""
);
...
<template>
  <form
    class="mt-2 d-flex"
    style="margin-left: 88px; margin-right: 44px; gap: 16px"
  >
    <!-- General Information -->
    ...
    <div class="d-flex flex-column" style="width: 50%; gap: 12px">
      <div
        ref="form"
        width="100%"
        class="text-start py-2 pl-5 pr-5 rounded-lg d-flex justify-center"
        style="border: solid 1px #80ccf9"
      >
        <v-card-text
          class="pa-0 d-flex flex-column"
          style="gap: 10px; height: 500px; overflow: scroll"
        >
          <div class="d-flex justify-space-between" style="gap: 30px">
            ...
            <h1
              class="text-primary font-weight-regular mr-16"
              style="font-size: 24px"
            >
              {{ numberOfItems }} Items
            </h1>
